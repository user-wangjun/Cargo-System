...
export class InventoryService {
  ...
  async balances(query: InventoryBalanceQueryDto, companyId: string) {
    const page = Math.max(1, Number(query.page ?? 1));
    const pageSize = Math.max(1, Math.min(200, Number(query.pageSize ?? 20)));
    const offset = (page - 1) * pageSize;

    const where: string[] = ['ib.company_id = $1'];
  ...
  async ledger(query: InventoryLedgerQueryDto, companyId: string) {
    const page = Math.max(1, Number(query.page ?? 1));
    const pageSize = Math.max(1, Math.min(200, Number(query.pageSize ?? 20)));
    const offset = (page - 1) * pageSize;

    const where: string[] = ['il.company_id = $1'];
    const params: unknown[] = [companyId];
    ...
    if (query.dateFrom) {
      ...
      where.push(`il.occurred_at >= $${params.length}::timestamptz`);
    }
    if (query.dateTo) {
    ...
    const countRows = await this.dataSource.query(`select count(*)::int as total from inventory_ledger il ${whereSql}`, params);
    const total = (countRows[0] as { total: number })?.total ?? 0;
  ...
  async adjust(dto: CreateInventoryAdjustmentDto, companyId: string, actorUserId?: string) {
    ...
    try {
      ...
      const ledgerId = (insertedLedger[0] as { id: string }).id;
