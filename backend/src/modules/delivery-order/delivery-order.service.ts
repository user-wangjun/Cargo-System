...
export class DeliveryOrderService {
  ...
  async create(dto: CreateDeliveryOrderDto, companyId: string, actorUserId?: string) {
  ...
  async update(id: string, dto: UpdateDeliveryOrderDto, companyId: string, actorUserId?: string) {
    ...
    assertAllowedStatus('delivery order', 'edit', currentStatus, ['DRAFT', 'PRINTED', 'UPDATED_AFTER_PRINT']);
  ...
  async post(id: string, companyId: string, actorUserId?: string) {
    const runner = this.dataSource.createQueryRunner();
    await runner.connect();
    await runner.startTransaction();
    try {
      ...
      assertAllowedStatus('delivery order', 'post', header.status, ['DRAFT']);
  ...
  async print(id: string, companyId: string, actorUserId?: string) {
    const currentRows = await this.dataSource.query('select status from delivery_orders where id = $1 and company_id = $2 limit 1', [id, companyId]);
    if (currentRows.length === 0) {
      throw new NotFoundException('Delivery order not found');
    }
    const currentStatus = (currentRows[0] as { status: string }).status;
    assertAllowedStatus('delivery order', 'print', currentStatus, ['POSTED', 'UPDATED_AFTER_PRINT']);
    ...
    if (result.length === 0) {
      throw new BadRequestException('Failed to print delivery order');
    }
  ...
  async voidDoc(id: string, companyId: string, actorUserId?: string) {
    ...
    try {
      ...
      const headerRows = await runner.query(
        `
        select id, delivery_no, warehouse_id, status
        from delivery_orders
        where id = $1 and company_id = $2
        for update
        `,
      ...
      const header = headerRows[0] as { id: string; delivery_no: string; warehouse_id: string; status: string };
      assertAllowedStatus('delivery order', 'void', header.status, ['DRAFT', 'POSTED']);
      ...
      if (header.status === 'POSTED') {
        ...
        for (const item of items as Array<{ id: string; product_id: string; converted_base_qty: unknown }>) {
          const restoreQty = toNumber(item.converted_base_qty);
          const balanceRows = await runner.query(
            `
            select on_hand_qty, reserved_qty
            from inventory_balances
          ...
          const newOnHand = currentOnHand + restoreQty;
          const newAvailable = newOnHand - currentReserved;
          ...
          await runner.query(
            ...
            do update set
              ...
              updated_at = now()
            `,
            [header.warehouse_id, item.product_id, newOnHand, currentReserved, newAvailable, companyId]
          );
          ...
          await runner.query(
            ...
            insert into inventory_ledger
              (warehouse_id, product_id, biz_type, biz_no, biz_line_id, change_qty, balance_qty, created_by, company_id)
            values
              ($1, $2, 'VOID_OUTBOUND', $3, $4, $5, $6, $7, $8)
            `,
            [header.warehouse_id, item.product_id, header.delivery_no, item.id, restoreQty, newOnHand, SYSTEM_USER_ID, companyId]
          );
        }
      }
      ...
      const result = await runner.query(
        `
        update delivery_orders
        set status = 'VOID', printed_at = null, updated_at = now()
        where id = $1 and company_id = $2
        returning id
        `,
        [id, companyId]
      ...
      if (result.length === 0) {
        throw new BadRequestException('Failed to void delivery order');
      }
