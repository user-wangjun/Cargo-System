...
export class PurchaseReceiptService {
  ...
  async create(dto: CreatePurchaseReceiptDto, companyId: string, actorUserId?: string) {
  ...
  async update(id: string, dto: UpdatePurchaseReceiptDto, companyId: string, actorUserId?: string) {
    ...
    assertAllowedStatus('purchase receipt', 'edit', currentStatus, ['DRAFT']);
  ...
  async post(id: string, companyId: string, actorUserId?: string) {
    ...
    try {
      ...
      assertAllowedStatus('purchase receipt', 'post', header.status, ['DRAFT']);
  ...
  async voidDoc(id: string, companyId: string, actorUserId?: string) {
    ...
    const currentStatus = (currentRows[0] as { status: string }).status;
    assertAllowedStatus('purchase receipt', 'void', currentStatus, ['DRAFT']);
    ...
    if (result.length === 0) {
      throw new BadRequestException('Failed to void purchase receipt');
    }
