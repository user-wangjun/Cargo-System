...
export class PurchaseReceiptController {
  ...
  async create(@Req() req: AuthReq, @Body() dto: CreatePurchaseReceiptDto) {
    return { code: 0, message: 'ok', data: await this.purchaseReceiptService.create(dto, this.companyId(req), this.userId(req)) };
  }
  ...
  @Patch(':id')
  @RequirePermissions('inventory.edit')
  async update(@Req() req: AuthReq, @Param('id') id: string, @Body() dto: UpdatePurchaseReceiptDto) {
    return { code: 0, message: 'ok', data: await this.purchaseReceiptService.update(id, dto, this.companyId(req), this.userId(req)) };
  }

  @Post(':id/post')
  @RequirePermissions('inventory.edit')
  async post(@Req() req: AuthReq, @Param('id') id: string) {
    return { code: 0, message: 'ok', data: await this.purchaseReceiptService.post(id, this.companyId(req), this.userId(req)) };
  }

  @Post(':id/void')
  @RequirePermissions('inventory.edit')
  async voidDoc(@Req() req: AuthReq, @Param('id') id: string) {
    return { code: 0, message: 'ok', data: await this.purchaseReceiptService.voidDoc(id, this.companyId(req), this.userId(req)) };
  }
}
